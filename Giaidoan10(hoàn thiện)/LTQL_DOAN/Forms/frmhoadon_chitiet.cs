...
namespace LTQL_DOAN.Forms
{
    public partial class frmhoadon_chitiet : Form
    {
        ...
        {
            // Đặt giới hạn cho NumericUpDown
            numdongiaban.Maximum = 100_000_000;
            numdongiaban.Minimum = 0;
            numsoluong.Maximum = 10_000;
            ...
            try
            {
                using (var db = new QUANLYCHSDbContext())
                {
                    ...
                    if (!isNew)
                    {
                        ...
                        // Hoàn trả số lượng vào kho trước khi xóa chi tiết cũ
                        if (hd.ChiTietHoaDon != null)
                        {
                            foreach (var oldItem in hd.ChiTietHoaDon)
                            {
                                var khoRecord = db.Kho.FirstOrDefault(k => k.SachID == oldItem.SachID);
                                if (khoRecord == null && oldItem.Sach != null)
                                {
                                    khoRecord = db.Kho.FirstOrDefault(k => k.MaSach == oldItem.Sach.MaSach);
                                }

                                if (khoRecord != null)
                                {
                                    khoRecord.SoLuong += oldItem.SoLuongBan;
                                }
                            }
                        }
                    ...
                    foreach (var item in hoaDonChiTiet)
                    {
                        ...
                        var khoItem = db.Kho.FirstOrDefault(k => k.SachID == item.SanPhamID);
                        if (khoItem == null)
                        {
                            var s = db.Sach.Find(item.SanPhamID);
                            if (s != null) khoItem = db.Kho.FirstOrDefault(k => k.MaSach == s.MaSach);
                        }

                        if (khoItem != null)
                        {
                            khoItem.SoLuong -= item.SoLuongBan;
                        }
                    }
        ...
        private void btnxacnhan_Click(object sender, EventArgs e)
        {
            ...
            // Kiểm tra tồn kho trước khi thêm vào danh sách
            using (var db = new QUANLYCHSDbContext())
            {
                var khoRecord = db.Kho.FirstOrDefault(k => k.SachID == maSanPham);
                if (khoRecord == null)
                {
                    var s = db.Sach.Find(maSanPham);
                    if (s != null) khoRecord = db.Kho.FirstOrDefault(k => k.MaSach == s.MaSach);
                }

                int tonKho = khoRecord != null ? khoRecord.SoLuong : 0;
                
                // Nếu là chỉnh sửa dòng đã có trong grid, ta cộng tạm số lượng cũ vào để kiểm tra
                var existingInGrid = hoaDonChiTiet.FirstOrDefault(x => x.SanPhamID == maSanPham);
                int currentInGrid = existingInGrid != null ? existingInGrid.SoLuongBan : 0;

                if (tonKho + currentInGrid < numsoluong.Value)
                {
                    MessageBox.Show($"Số lượng trong kho không đủ (Hiện có: {tonKho})!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
